...
bool BeliefSet::beliefComp::operator()(const Belief* belief1, const Belief* belief2) const
{
    ...
    {
        if (b1->belief[i].pathLength < b2->belief[i].pathLength) 
            return true;
        else if (b1->belief[i].pathLength > b2->belief[i].pathLength) 
            return false;
        for (long j=0; j < (long)b1->belief[i].state.size(); j++)
        {
            if ( b1->belief[i].state[j] <  b2->belief[i].state[j]) 
                return true;
            else if ( b1->belief[i].state[j] >  b2->belief[i].state[j]) 
                return false;
        }
    }
    return false;
